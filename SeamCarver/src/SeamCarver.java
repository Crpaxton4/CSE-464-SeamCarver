	...
	static float[][] dual_gradient_energy(ImageData img){
		...
		for(int col = 0; col < W; col++) { // j
			for(int row = 0; row < H; row++) { // i
				...
				energyMap[col][row] = (float)( Math.pow(redGradient[0], 2) + Math.pow(redGradient[0], 2) +
											   Math.pow(blueGradient[0], 2) + Math.pow(blueGradient[0], 2) +
											   Math.pow(greenGradient[0], 2) + Math.pow(greenGradient[0], 2));
